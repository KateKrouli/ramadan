(() => {
  ...
  var app_default = '<div id="app" class="app">\n  <div class="app__overlay">\n    {{dropdown}}\n    {{timer}}\n    {{quote}}\n  </div>\n</div>\n';

  // src/components/timer.html
  var timer_default = '<div id="timer" class="timer">00:00:00</div>\n';

  // src/components/dropdown.html
  var dropdown_default = '<select id="locationSelect" class="dropdown"></select>\n';

  // src/components/quote.html
  var quote_default = '<div id="quote" class="quote"></div>\n';

  // src/js/config/locations.js
  var LOCATIONS = [
    { label: "Mecca", query: "Mecca, Saudi Arabia" },
    { label: "Dubai", query: "Dubai, UAE" },
    { label: "Cairo", query: "Cairo, Egypt" },
    { label: "Istanbul", query: "Istanbul, Turkey" },
    { label: "Jakarta", query: "Jakarta, Indonesia" },
    { label: "Almaty", query: "Almaty, Kazakhstan" }
  ];

  // src/js/ui/dropdown.js
  function initDropdown(onChange) {
  ...
  function setBackground(mode) {
    ...
    app.style.backgroundImage = mode === "night" ? "url(/assets/night.jpg)" : "url(/assets/day.jpg)";
  }
  ...
  function startTimer(target, onTick, onEnd) {
    function tick() {
      ...
      const h = Math.floor(diff2 / 36e5);
      const m = Math.floor(diff2 % 36e5 / 6e4);
      const s2 = Math.floor(diff2 % 6e4 / 1e3);
      onTick(
        `${h.toString().padStart(2, "0")}:${m.toString().padStart(2, "0")}:${s2.toString().padStart(2, "0")}`
      );
    }
    tick();
    return setInterval(tick, 1e3);
  }
  ...
  // src/js/main.js
  document.getElementById("root").innerHTML = app_default.replace("{{timer}}", timer_default).replace("{{dropdown}}", dropdown_default).replace("{{quote}}", quote_default);
  var timerId = null;
